...
export default function IndividualRegistrationForm() {
  ...
  const [teamIDs, setTeamIDs] = useState<{id: string; name: string}[]>([{id: "", name: ""}]); // For storing team IDs
  const [selectedTeamID, setSelectedTeamID] = useState<string>(""); // For storing team IDs

  const [personType, setPersonType] = useState<"player" | "coach" | "chaperone">("player");

  let sectionName: "coaches" | "chaperones" | "players" = (personType === "coach") ? "coaches" :
                          (personType === "chaperone") ? "chaperones" :
                          (personType === "player") ? "players" : "players"
  let columnType = (personType === "coach") ? coachFieldMetadata :
                          (personType === "chaperone") ? chaperoneFieldMetadata :
                          (personType === "player") ? playerFieldMetadata : playerFieldMetadata
  let emptyFormType = (personType === "coach") ? emptyCoachForm :
                          (personType === "chaperone") ? emptyChaperoneForm :
                          (personType === "player") ? emptyPlayerForm : emptyPlayerForm
  ...
  useEffect(() => { 
    ...
    sectionName = (personType === "coach") ? "coaches" :
                        (personType === "chaperone") ? "chaperones" :
                        (personType === "player") ? "players" : "players"

    columnType = (personType === "coach") ? coachFieldMetadata :
                        (personType === "chaperone") ? chaperoneFieldMetadata :
                        (personType === "player") ? playerFieldMetadata : playerFieldMetadata
    emptyFormType = (personType === "coach") ? emptyCoachForm :
                        (personType === "chaperone") ? emptyChaperoneForm :
                        (personType === "player") ? emptyPlayerForm : emptyPlayerForm
    setErrors({});
    setFormData(emptyFormType);
  ...
  newValue: string | File | boolean | number | null // accepts files
    ...
    setErrors((prevErrors) => ({
      ...
      setFormData((prev) => ({ // update the specific info
        ...prev, [singleFieldMeta.field_name]: newValue, 
      }));
  };
   ...
   */
    ...
    columnType.forEach((singleFieldMeta) => {
      ...
      const value = formData[singleFieldMeta.field_name as keyof typeof formData];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await addPersonToDatabase(formData, sectionName, selectedTeamID); // Submit Individual
    if (result.error) {
      console.error("Error submitting form:", result.error);
      alert("There was an error submitting the form. Please try again.");
      return;
    } else {
      console.log("Successfully Added", personType, formData.first_name, formData.last_name, "to team", teamIDs.find(team => team.id === selectedTeamID));
    ...
    <div className="py-15 w-[100%] mx-auto p-[4%] bg-white shadow-lg rounded-lg">
      ...
      {!isSubmitted ? ( // Show form if it has NOT been submitted yet
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* PLAYER INFO SECTION */}
            <div>
              <h3 className="text-lg font-semibold">Flagway {personType.charAt(0).toUpperCase() + personType.slice(1)} Information</h3>
              <p className="text-lg font-semibold">(Required *)</p>
              <div>
                {createExpandedFormSection(sectionName, columnType, 0, handleChange, formData, errors)}
              </div>
            </div>
          ...
          <div className="text-center">
            <p className="text-lg font-semibold text-gray-900">
              Thank you! {`${formData.first_name} ${formData.last_name} has been successfully registered.`}
            </p>
            <button
              onClick={resetForm}
