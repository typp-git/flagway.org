      ...
      <Container className="text-white">
        ...
        <div className="h-30 my-6 w-full flex flex-row items-center">
          ...
          >
            ...
            <div> <span className="text-blue-300">State:</span> {state}</div>
          </div>
        ...
        <h2 className= "italic">PLAYERS</h2>

        {players && players.length > 0 ? (
          players.map((player: Player) => (

            <div className="flex flex-col md:flex-row
              my-6 max-w-xl rounded-lg 
              items-center overflow-hidden
              bg-gray-800 bg-[url('/structures.png')] 
              bg-cover bg-center
              hover:scale-105
              transform
              ...
              key={player.name}
            >
              <div className="relative w-1/2h-full md:w-full md:h-1/2 object-cover group" >
                ...
                <div className="flex items-center justify-center aspect-square overflow-hidden 
                  ...
                  "
                >
                  <Image src={player.grade % 3 == 1? DefaultProfilePic1 : player.grade % 3 == 2? DefaultProfilePic2 : DefaultProfilePic3} alt="Profile Picture"/>
                </div>
              </div>

              <div className="h-full w-1/2 md:h-1/2 md:w-full py-6 px-6 text-white text-left italic">
                    <h2 className="group uppercase">
                      {player.name}
                    </h2>
                    ...
                    <div>
                      ...
                      <span className="text-blue-300"> Years at YPP:</span> <span className="text-1xl">{player.yearsYPP}</span>
                    </div>
                    <div>
                      {/* <span className="text-2xl">{player.grade}</span><span className="text-blue-300">th Grade</span> */}
                      <span className="text-blue-300"> Grade: </span> <span className="text-1xl">{player.grade}</span>
                    </div>
                    <div>
                      <span className="text-blue-300"> City: </span> <span className="text-1xl">{player.city}</span>
                    </div>
              </div>
